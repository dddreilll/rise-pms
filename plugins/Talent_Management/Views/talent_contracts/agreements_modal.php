<div class="modal-body clearfix">
    <div class="container-fluid">
        ...
        <?php if (!$states) { ?>
            <div class="alert alert-info mb0"><i data-feather="info" class="icon-16"></i> <?php echo app_lang("talent_contract_agreements_none"); ?></div>
        <?php } else { ?>
            <div class="table-responsive">
                <table class="table table-hover mb0" id="talent-agreements-table">
                    <thead>
                        <tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <?php if ($state["required"]) { ?>
                                        <span class="badge badge-light ml5"><?php echo app_lang("talent_contract_required_label"); ?></span>
                                    <?php } else { ?>
                                        <span class="text-off ml5"><?php echo app_lang("talent_contract_optional_label"); ?></span>
                                    <?php } ?>
                                </td>
                                <td>
                                    <?php if ($contract) { ?>
                                ...
                                <td class="text-end" style="white-space: nowrap;">
                                    ...
                                    if ($state["can_send"]) {
                                        echo " " . talent_contract_send_action_html($talent_project_id, $state["template_id"], $state["status"] !== "");
                                    }
                                    if ($state["status"] !== "signed" && $state["template_id"]) {
                                        echo " " . talent_contract_paper_action_html($talent_project_id, $state["template_id"]);
                                    }
                                    ?>
                                </td>
                            </tr>
